...
public class WeaponLoader extends AssetLoader<Weapon> {
    ...
    public void loadAsset(String key, InputStream in) {
        try {
            ...
            int[] timings = null;
            BufferedImage image = null, himage = null, projectileImage = null;
            ...
            for (int i=0; i<src.length; i++) {
                if (src[i] == ';') {
                    if (property.equals("cooldown")) {
                        ...
                        buffer = "";
                    } else if (property.equals("hImage")) {
                        System.out.println("hImage is athing");
                        System.out.println("It has the key " + buffer);
                        himage = assetManager.getImage(buffer);
                        buffer = "";
                        ...
                        buffer = "";
                    } else if (property.equals("projectile")) {
                        projectileImage = assetManager.getImage(buffer);
                        buffer = "";
                    }
                } else if (src[i] == ':') {
                    property = buffer;
            ...
            add(key, new Weapon(key, attack, effect, timings, 
                    cooldown, image, himage, sound, 
                    attackAnimationTrack, projectileImage));
